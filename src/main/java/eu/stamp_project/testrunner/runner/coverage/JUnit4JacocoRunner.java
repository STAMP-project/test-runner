...
public class JacocoRunner {
    ...
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        if (args.length > 3 && args[2].equals(JUnit4Runner.BLACK_LIST_OPTION)) {
            jacocoRunner = new JacocoRunner(classesDirectory, testClassesDirectory, Arrays.asList(args[3].split(JUnit4Runner.PATH_SEPARATOR)));
        } else {
    ...
    private Coverage run(String classesDirectory,
        ...
        try {
            ...
            if (EntryPoint.jUnit5Mode) {
                ...
                JUnit5Runner.run(fullQualifiedNameOfTestClass, Arrays.asList(testMethodNames), (JUnit5Coverage)listener, this.instrumentedClassLoader);
            } else {
                listener = new JUnit4Coverage();
                JUnit4Runner.run(fullQualifiedNameOfTestClass, Arrays.asList(testMethodNames), (JUnit4Coverage)listener, this.instrumentedClassLoader);
            }
    ...
    private Coverage run(String classesDirectory,
        ...
        try {
            ...
            if (EntryPoint.jUnit5Mode) {
                listener = new JUnit5Coverage();
                JUnit5Runner.run(Arrays.asList(fullQualifiedNameOfTestClasses), this.blackList, (JUnit5Coverage)listener, this.instrumentedClassLoader);
            } else {
                listener = new JUnit4Coverage();
                JUnit4Runner.run(Arrays.asList(fullQualifiedNameOfTestClasses), this.blackList, (JUnit4Coverage)listener, this.instrumentedClassLoader);
            }
            if (!((TestListener)listener).getFailingTests().isEmpty()) {
                System.err.println("Some test(s) failed during computation of coverage:\n" +
                        ((TestListener)listener).getFailingTests()
